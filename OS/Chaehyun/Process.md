...
- 사실 여러 개의 프로세스가 동시에 실행되는 것은 아님
- TIME SHARING을 통해서 빠른 속도로 왔다갔다 하면서 프로세스들을 처리 ⇒ 여러개가 동시에 돌아가는 것처럼 보임
- 프로세스들이 교체되고, 실행되는 과정에서 다음 프로세스, 이전 프로세스들을 빨리 가져와야 함
    
    ⇒ PCB를 이용해서 다른 프로세스들의 작업을 기억하고 가져옴
...
<img width="827" height="761" alt="image" src="https://github.com/user-attachments/assets/608c24b9-ffff-4a95-8598-6a86c7e61751" />
- Process ID (PID): 프로세스 고유 번호
- Process State: 생성, 준비, 실행, 대기, 완료의 상태 정보
- Program Counter (Pointer): 프로세스가 다음에 실행할 명령어의 주소
- CPU Register: CPU 옆에 붙어있는 가장 작은 단위의 임시 메모리 저장 공간
- CPU Scheduling information: CPU스케쥴링에서 어떤 우선순위를 가지고 있는지에 대한 정보
- Memory-management information: 메모리가 할당된 자원의 정보
- Accounting information: 최종 실행 시각, CPU를 점유하고 있는 시간
...
- 스위칭 과정
    ...
    <img width="1135" height="887" alt="image" src="https://github.com/user-attachments/assets/b3164d73-7365-4bf8-9b44-5f3be3f5bf5b" />

    
    1. CPU는 Process P0을 실행한다 (Executing)
    2. 일정 시간이 지나 Interupt 또는 System call이 발생한다 (CPU는 idle 상태)
    3. 현재 실행중인 Process P0의 상태를 PCB0에 저장한다 (save state into PCB0)
    4. 다음으로 실행할 Process P1을 선택한다 (CPU 스케줄링)
    5. Process P1의 상태를 PCB1에서 불러온다 (reload state from PCB1)
    6. 일정시간이 지나 Interrupt 또는 System call이 발생한다 (CPU는 idle 상태)
    7. 현재 실행 중인 Process P1의 상태를 PCB1에 저장한다 (save state into PCB1)
    8. 다시 Process P0을 실행할 차례가 된다 (CPU 스케줄링)
    9. Process P0의 상태를 PCB0에서 불러온다 (reload state from PCB0)
    10. CPU는 Process P0을 중간 시점부터 실행한다 (Executing)
...
모두 멀티태스킹 환경에서 여러 프로세스 또는 스레드를 동시에 실행하기 위한 기술

- TCB가 PCB보다 가벼움
    - 스레드 컨텍스트 스위칭이 프로세스 컨텍스트 스위칭보다 더 빠름
    - TCB는 stack 및 간단한 register 포인터 정보만을 저장하기 때문에 PCB보다 가벼워 더 빨리 읽고 쓸 수 있음
- 캐시 메모리 초기화 여부
    - CPU 캐시 메모리
        - CPU와 메인 메모리 사이에 위치하며 CPU에서 한번 이상 읽어들인 메모리의 데이터를 저장하고 있다가
        - CPU가 다시 그 메모리에 저장된 데이터를 요구할 때, 메인 메모리를 통하지 않고 곧바로 데이터를 전달해주는 용도
    - 프로세스 컨텍스트 스위칭이 일어날 경우,
        
        다른 프로세스 실행으로 인해 CPU가 새로운 명령어와 데이터를 로드해야 하기 때문에 CPU 캐시 메모리 초기화해야 함
        
        ⇒ 부담
        
    - 스레드 컨텍스트 스위칭이 일어날 경우,
        
        프로세스 내 스레드 간에 스택과 레지스터 값 등 일부 컨텍스트 정보만 변경되므로 CPU 캐시 메모리는 초기화되지 않음
        
        but, 다른 CPU 코어에서 실행될 때는 해당 코어의 캐시 메모리에 스레드 컨텍스트 정보가 로드되어야 하므로 초기화 가능
        
- 자원 동기화 문제
    - 스레드 컨텍스트 스위칭이 발생해 다른 스레드가 heap 영역의 공유 데이터에 접근할 때,
    이전 스레드가 이미 공유 자원을 사용하고 있는 경우 동기화 문제 발생 가능
        
        ⇒ 스레드 간에 경쟁 조건
        
    - 프로세스는 기본적으로 독립된 공간이지만, IPC와 같은 공유 자원을 사용하는 경우에 똑같이 경쟁 조건이 발생 가능
